...
default_all(void)
{
	...
	default_line("tcp_outgoing_address 255.255.255.255");
	...
	/* No default for authenticate_program */
	default_line("authenticate_children 5");
	default_line("authenticate_ttl 3600");
	default_line("authenticate_ip_ttl 0");
	/* No default for wais_relay_host */
	default_line("wais_relay_port 0");
	default_line("request_header_max_size 10 KB");
	default_line("request_body_max_size 1 MB");
	default_line("reply_body_max_size 0");
	/* No default for refresh_pattern */
#if HEAP_REPLACEMENT
	default_line("replacement_policy LFUDA");
#endif
	default_line("reference_age 1 year");
	...
	default_line("siteselect_timeout 4 seconds");
	default_line("read_timeout 15 minutes");
	default_line("request_timeout 30 seconds");
	...
	default_line("store_objects_per_bucket 50");
	...
	/* No default for anonymize_headers */
	/* No default for fake_user_agent */
...
defaults_if_none(void)
{
	...
	if (check_null_wordlist(Config.hierarchy_stoplist))
		default_line("hierarchy_stoplist cgi-bin ?");
...
parse_line(char *buff)
{
	...
	if ((token = strtok(buff, w_space)) == NULL)
		...
		parse_address(&Config.Addrs.tcp_outgoing);
		...
		parse_wordlist(&Config.Program.authenticate);
	else if (!strcmp(token, "authenticate_children"))
		parse_int(&Config.authenticateChildren);
	else if (!strcmp(token, "authenticate_ttl"))
		parse_int(&Config.authenticateTTL);
	else if (!strcmp(token, "authenticate_ip_ttl"))
		parse_int(&Config.authenticateIpTTL);
		...
		parse_b_size_t(&Config.maxReplyBodySize);
	else if (!strcmp(token, "refresh_pattern"))
		parse_refreshpattern(&Config.Refresh);
#if HEAP_REPLACEMENT
	else if (!strcmp(token, "replacement_policy"))
		parse_string(&Config.replPolicy);
#endif
	else if (!strcmp(token, "reference_age"))
		parse_time_t(&Config.referenceAge);
	...
	else if (!strcmp(token, "siteselect_timeout"))
		parse_time_t(&Config.Timeout.siteSelect);
		...
		parse_peer_access();
	else if (!strcmp(token, "proxy_auth_realm"))
		parse_eol(&Config.proxyAuthRealm);
#if USE_IDENT
	...
	else if (!strcmp(token, "anonymize_headers"))
		parse_http_header(&Config.anonymize_headers);
	else if (!strcmp(token, "fake_user_agent"))
		parse_eol(&Config.fake_ua);
...
dump_config(StoreEntry *entry)
{
	...
	dump_address(entry, "tcp_outgoing_address", Config.Addrs.tcp_outgoing);
	...
	dump_wordlist(entry, "authenticate_program", Config.Program.authenticate);
	dump_int(entry, "authenticate_children", Config.authenticateChildren);
	dump_int(entry, "authenticate_ttl", Config.authenticateTTL);
	dump_int(entry, "authenticate_ip_ttl", Config.authenticateIpTTL);
	...
	dump_b_size_t(entry, "reply_body_max_size", Config.maxReplyBodySize);
	dump_refreshpattern(entry, "refresh_pattern", Config.Refresh);
#if HEAP_REPLACEMENT
	dump_string(entry, "replacement_policy", Config.replPolicy);
#endif
	dump_time_t(entry, "reference_age", Config.referenceAge);
	...
	dump_time_t(entry, "siteselect_timeout", Config.Timeout.siteSelect);
	...
	dump_eol(entry, "proxy_auth_realm", Config.proxyAuthRealm);
#if USE_IDENT
	...
	dump_http_header(entry, "anonymize_headers", Config.anonymize_headers);
	dump_eol(entry, "fake_user_agent", Config.fake_ua);
...
free_all(void)
{
	...
	free_address(&Config.Addrs.tcp_outgoing);
	...
	free_wordlist(&Config.Program.authenticate);
	free_int(&Config.authenticateChildren);
	free_int(&Config.authenticateTTL);
	free_int(&Config.authenticateIpTTL);
	...
	free_b_size_t(&Config.maxReplyBodySize);
	free_refreshpattern(&Config.Refresh);
#if HEAP_REPLACEMENT
	free_string(&Config.replPolicy);
#endif
	free_time_t(&Config.referenceAge);
	...
	free_time_t(&Config.Timeout.siteSelect);
	...
	free_eol(&Config.proxyAuthRealm);
#if USE_IDENT
	...
	free_http_header(&Config.anonymize_headers);
	free_eol(&Config.fake_ua);
