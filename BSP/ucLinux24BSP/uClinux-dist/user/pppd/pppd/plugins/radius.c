...
{
	if (attrib->length != 4) {
		error("RADIUS: invalid attribute length");
		return 0;
	}
	else {
		return 1;
...
radius_auth(struct radius_attrib **attriblist, chap_state *cstate)
{
	...
	if (ret < 0) {
			...
			if (attrib->vendor == PW_VENDOR_NONE) {
				...
				case PW_SERVICE_TYPE:
					if (ntohl(attrib->u.value) != PW_FRAMED_USER) {
						error("RADIUS: service type is not framed");
						ret = 0;
					}
					break;
				...
				case PW_FRAMED_PROTOCOL:
					if (ntohl(attrib->u.value) != PW_PPP) {
						error("RADIUS: framed protocol is incorrect");
						ret = 0;
					}
					break;
				...
				case PW_MS_CHAP2_SUCCESS:
					...
					if (attrib->length != 43) {
						error("RADIUS: invalid MS_CHAP2_SUCCESS length");
						ret = 0;
						break;
					}
				...
				case PW_MS_CHAP_MPPE_KEYS: {
					...
					if (attrib->length != 32) {
						error("RADIUS: invalid MS_CHAP_MPPE_KEYS length");
						ret = 0;
						break;
					}
				...
				case PW_MS_MPPE_SEND_KEY:
					...
					if (attrib->length != 34) {
						error("RADIUS: invalid MS_MPPE_SEND_KEY length");
						ret = 0;
						break;
					}
					if ((attrib->u.string[0] & 0x80) == 0) {
						error("RADIUS: invalid MS_MPPE_SEND_KEY salt");
						ret = 0;
						break;
					}
					if (attrib->u.string[2] != 16) {
						error("RADIUS: invalid MS_MPPE_SEND_KEY keylength");
						ret = 0;
						break;
					}
				...
				case PW_MS_MPPE_RECV_KEY:
					...
					if (attrib->length != 34) {
						error("RADIUS: invalid MS_MPPE_RECV_KEY length");
						ret = 0;
						break;
					}
					if ((attrib->u.string[0] & 0x80) == 0) {
						error("RADIUS: invalid MS_MPPE_RECV_KEY salt");
						ret = 0;
						break;
					}
					if (attrib->u.string[2] != 16) {
						error("RADIUS: invalid MS_MPPE_RECV_KEY keylength");
						ret = 0;
						break;
					}
		...
		error("RADIUS: server sent unexpected response");
		ret = 0;
	}
...
radius_chap_auth(char *user, u_char *remmd, int remmd_len, chap_state *cstate)
{
	...
	case CHAP_DIGEST_MD5:
		if (remmd_len != MD5_SIGNATURE_SIZE) {
			error("RADIUS: invalid CHAP response length");
			goto error;
		}
	...
	case CHAP_MICROSOFT: {
		...
		if (remmd_len != MS_CHAP_RESPONSE_LEN) {
			error("RADIUS: invalid MSCHAP response length");
			goto error;
		}
	...
	case CHAP_MICROSOFT_V2: {
		...
		if (remmd_len != MS_CHAP_RESPONSE_LEN) {
			error("RADIUS: invalid MSCHAPv2 response length");
			goto error;
		}
	...
	default:
		error("RADIUS: unsupported challenge type");
		goto error;
	}
