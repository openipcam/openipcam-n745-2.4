...
{
	int	fd, fdcp = -1, binary, erase, preserve, dolock, dounlock, donftl, doforce;
	int	size, pos;
	long	offset;
	...
	if (donftl) {
		...
		if (ioctl(fd, BLKGETSIZE, &l) < 0) {
			printf("netflash: ioctl(BLKGETSIZE) failed, errno=%d\n", errno);
			exit(1);
		}
	...
	if (pbuf == (char *) NULL) {
		printf("ERROR: could not allocate %ld bytes for buffer, errno=%d\n",
				sector_size, errno);
		exit(1);
	}
	...
	for (; pos + sector_size <= device_size; pos += sector_size) {
		...
		if (preserve) {
			...
			if (read(fd, pbuf, sector_size) != sector_size)
				printf("ERROR: read(pos=%x) failed, errno=%d\n", pos, errno);
		}
		...
		if (erase) {
			...
			if (ioctl(fd, MEMERASE, &erase_info) == -1)
				printf("ERROR: ioctl(MEMERASE) failed, pos=%x, errno=%d\n",
						pos, errno);
#else
 			if (ioctl(fd, BMSERASE, pos) < 0)
 				printf("ERROR: ioctl(BMERASE) failed, pos=%x, errno=%d\n",
						pos, errno);
#endif
		}
		...
		for (sp = pbuf; sp < pbuf + sector_size; sp += write_chunk)
			if (write(fd, sp, write_chunk) != write_chunk)
				printf("ERROR: write(size=%d) failed, errno=%d\n",
						write_chunk, errno);
