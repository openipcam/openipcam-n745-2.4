...
void slip_start(void)
{
    ...
    if (netmask) {
	    ...
	    PATH_IFCONFIG,link_iface,local_ip,remote_ip,netmask,mtu, metric+1);
    } else {
        sprintf(buf,"%s sl%d %s pointopoint %s mtu %d metric %d up",
	    PATH_IFCONFIG,link_iface,local_ip,remote_ip,mtu, metric+1);
    }
    res = system(buf);
    report_system_result(res,buf);

    /* Set the routing for the new slip interface */
    set_ptp("sl",link_iface,remote_ip,metric+1);
