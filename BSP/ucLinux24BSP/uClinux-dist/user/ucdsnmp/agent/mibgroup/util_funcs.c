...
int get_exec_output(struct extensible *ex)
{
  ...
  if (curtime > (cachetime + EXCACHETIME) ||
    ...
    /* This stuff could be done in the child process but the embedded vfork
     * doesn't separate the address spaces so it wouldn't work there.
     */
