...
#ifdef EMBED
    /* No float conversions in embedded scanf routine */
    fscanf(in, "%d.%2d %d.%2d %d.%2d",
		    ipts, fpts, ipts + 1, fpts + 1, ipts + 2, fpts + 2);
    pave[0] = ipts[0] + fpts[0] * 0.01;
    pave[1] = ipts[1] + fpts[1] * 0.01;
    pave[2] = ipts[2] + fpts[2] * 0.01;
#else
				      ...
				      WriteMethod **write_method)
{
  ...
  switch (vp->magic) {
    case LOADAVE:
#ifdef EMBED
      f = avenrun[name[*length-1]-1];
      j = (int)f;
      sprintf(errmsg,"%d.%02d", j, (int)((f-j)*100)); 
#else
    ...
    case ERRORMSG:
      if (maxload[name[*length-1]-1] != 0 &&
        ...
        f = avenrun[name[*length-1]-1];
        j = (int)f;
        sprintf(errmsg,"%d min Load Average too high (= %d.%02d)",
