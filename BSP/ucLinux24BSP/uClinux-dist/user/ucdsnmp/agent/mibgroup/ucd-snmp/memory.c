...
#ifdef EMBED
/* Since we don't have swap in this environment, there is no reason
 * to provide a default value that we'll always be below.
 */
#define DEFAULTMINIMUMSWAP -1  	  /* kilobytes */
#else
...
#ifdef EMBED
	/* We don't handle the scanf %n format very well so we do the
	 * necessary conversion and formatting ourselves.
	 */
	for (j=0; *p && *p != '\n' && j < MAX_COL; j++) {
		if (1 != sscanf(p, "%u", row[i]+j))
			break;
		while(*p && isdigit(*p)) p++;
		while(*p && !isdigit(*p) && *p != '\n') p++;
	}
