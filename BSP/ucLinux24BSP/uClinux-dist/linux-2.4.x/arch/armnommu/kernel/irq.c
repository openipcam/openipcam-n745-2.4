...
//	printk("desc->lck_cnt:%d\n",desc->lck_cnt);
	...
	if (desc->lck_jif == jiffies &&
		...
		printk("jiffies:%d\n",jiffies);
		printk("instr_ptr:%d\n",instr_ptr);
		printk("desc->lck_pc:%d\n",desc->lck_pc);
		printk("desc->lck_jif:%d\n",desc->lck_jif);
	#endif
		if (desc->lck_cnt > MAX_IRQ_CNT) {
			printk("status:%x\n",PORTC_ISR);
			printk(KERN_ERR "MCLI IRQ LOCK: IRQ%d is locking the system, disabled\n", irq);
			disable_irq(irq);
		}
	} else {
		desc->lck_cnt = 0;
 ...
 */
asmlinkage void do_IRQ(int irq0, struct pt_regs * regs)
{
	struct irqdesc * desc;
	struct irqaction * action;
	int cpu;
	...
	//int irq=18,ISR=0x40000;
	//int irq=30,ISR=0x40000000;
	unsigned int irq = irq0 >> 2;
...
extern void init_EBI(void);//lsshi 2003-12-29 16:40
...
{
	//CSR_WRITE(CAHCNF,0x0);/*Close Cache*/
	//CSR_WRITE(CAHCON,0x4);/*Flush Cache*/
	//while(CSR_READ(CAHCON)&0x4);
}
void FlushCache()
{
	//CSR_WRITE(CAHCON,0x4);/*Flush Cache*/
