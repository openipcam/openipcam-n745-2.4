...
int ixp425_pci_read(u32 addr, u32 cmd, u32* data)
{
	...
	int retval = 0;
#ifdef CONFIG_IXP425_PCI_ERRATA
	int i;
#endif
...
#ifndef CONFIG_IXP425_PCI_ERRATA
...
{  
	/*
	 * First things first...
	 * If someone built the kernel w/o bounce buffer support 
	 * and we have > 64MB of ram, we just die now and tell
	 * them to go fix it.
	 */
#ifndef CONFIG_IXP425_LARGE_SDRAM
	struct sysinfo i;
	si_meminfo(&i);

	if(((unsigned long long)(i.totalram) << PAGE_SHIFT) > SZ_64M)
		panic("IXP425 system contains > 64MB ram, but kernel does not have bounce buffer support\n");
#endif
