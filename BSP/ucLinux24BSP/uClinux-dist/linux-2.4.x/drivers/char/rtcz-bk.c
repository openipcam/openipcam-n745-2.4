		     ...
		     unsigned long arg)
{
	...
	//	printk("day:%d\n",day);
		...
		//CMOS_WRITE(save_control, RTC_CONTROL); //mcli
		save_control = CMOS_READ(RTC_CONTROL); //mcli add
		CMOS_WRITE((save_control&0x7F), RTC_CONTROL);//mcli add
...
//		CMOS_WRITE(save_freq_select, RTC_FREQ_SELECT);
	...
	//	printk("day:%d\n",day);
		...
		CMOS_WRITE((save_control|RTC_SET), RTC_CONTROL);
#else
		save_control = CMOS_READ(RTC_CONTROL); //REG B
		CMOS_WRITE((save_control&0x86), RTC_CONTROL);

		save_control = CMOS_READ(RTC_CONTROL); //REG B
		CMOS_WRITE((save_control|0x86), RTC_CONTROL);
		//printk("RTC_CONTROL:%x\n",CMOS_READ(RTC_CONTROL));
#endif
//		save_freq_select = CMOS_READ(RTC_FREQ_SELECT);
//		CMOS_WRITE((save_freq_select|RTC_DIV_RESET2), RTC_FREQ_SELECT);
...
static void get_rtc_time(struct rtc_time *rtc_tm)
{
	...
	//printk("rtc_tm->tm_mon k:%d\n",rtc_tm->tm_mon);
	//printk("rtc_tm->tm_mday k:%d\n",rtc_tm->tm_mday);
