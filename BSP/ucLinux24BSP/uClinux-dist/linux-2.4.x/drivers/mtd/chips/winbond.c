/*
 * MTD map driver for WINBOND compatible flash chips (non-CFI)
 */
...
#include <asm-armnommu/unaligned.h> //2004-11-2 11:17
...
#define CMD_READ_STATUS			0x0070	//Read status register
#define CMD_WORD_WRITE			0x0040	//write word ( 2 bytes)
#define CMD_RESET				0x00ff	//Read Array/Reset
#define CMD_READ_IDENTIFIER		0x0090	//Read identifier Codes
#define CMD_BLOCK_ERASE1		0x0020	//Block erase 1
#define CMD_BLOCK_ERASE2		0x00D0	//Block erase 2

#define CMD_UNLOCK_DATA1		0x0060
#define CMD_LOCK_BLOCK			0x0001
#define CMD_UNLOCK_BLOCK		0x00D0

#define NOCACHEABLE			0x80000000	

/* Manufacturers */
#define MANUFACTURER_WINBOND	0x00B0
#define MANUFACTURER_WINBOND_1  0xddda


/* WINBOND */
#define W28J160BT	0x00E9
#define W28J160TT	0x00E8
#define W28J800BT	0x00ED
#define W28J800TT	0x00EC
#define W28J320BT	0x00E3
#define W28J320TT	0x00E2
#define W19B320ABT   0x227E //mcli 2004-11-29 15:16

#define SR7_MASK	0x80
#define IDTC0_MASK	0x01

struct winbond_flash_private {
	int device_type;	
	int interleave;	
	int numchips;	
...
static int winbond_flash_read(struct mtd_info *, loff_t, size_t, size_t *,
			  u_char *);
static int winbond_flash_write(struct mtd_info *, loff_t, size_t, size_t *,
			   const u_char *);
static int winbond_flash_erase(struct mtd_info *, struct erase_info *);
static void winbond_flash_sync(struct mtd_info *);
static int winbond_flash_suspend(struct mtd_info *);
static void winbond_flash_resume(struct mtd_info *);
static void winbond_flash_destroy(struct mtd_info *);
static struct mtd_info *winbond_flash_probe(struct map_info *map);


static struct mtd_chip_driver winbond_flash_chipdrv = {
	probe: winbond_flash_probe,
	destroy: winbond_flash_destroy,
	name: "winbond_flash",
	module: THIS_MODULE
};
...
static const char im_name[] = "winbond_flash";
...
{
	const struct winbond_flash_private *private = map->fldrv_priv;
				...
				int interleave)
{
	...
	if ((interleave == 2) && (map->bankwidth == 4)) {
		...
		return (((read1 >> 16) & SR7_MASK) !=
			((read2 >> 16) & SR7_MASK)) ||
		       (((read1 & 0xffff) & SR7_MASK) !=
			((read2 & 0xffff) & SR7_MASK));
	}
	return ((wide_read(map, addr) & SR7_MASK) !=
		(wide_read(map, addr) & SR7_MASK));
#else
	return !((wide_read(map, addr) & SR7_MASK) == SR7_MASK);
#endif
}
...
#else
	struct winbond_flash_private *private = map->fldrv_priv;
	__u32 cmd = unlock ? CMD_UNLOCK_BLOCK : CMD_LOCK_BLOCK ;
		
	send_cmd(map,sect_addr,CMD_READ_STATUS);
	while (flash_is_busy(map, sect_addr, private->interleave));
	send_cmd(map,sect_addr,CMD_UNLOCK_DATA1);
	send_cmd(map,sect_addr,cmd);
	while (flash_is_busy(map, sect_addr, private->interleave));
	send_cmd(map, sect_addr, CMD_RESET);
				   ...
				   unsigned long sect_addr)
{
	...
	wide_write(map, CMD_RESET_DATA, 0);
#else
	send_cmd(map, sect_addr, CMD_READ_IDENTIFIER);	
	status = ( wide_read(map, sect_addr + (map->bankwidth * ADDR_LOCK_CONF)) & IDTC0_MASK ) ;
	send_cmd(map, sect_addr,CMD_RESET);		  
#endif	
	return status;
}

static int winbond_flash_do_unlock(struct mtd_info *mtd, loff_t ofs, size_t len,
			       int is_unlock)
{
	struct map_info *map;
...
static int winbond_flash_unlock(struct mtd_info *mtd, loff_t ofs, size_t len)
{
	return winbond_flash_do_unlock(mtd, ofs, len, 1);
}

static int winbond_flash_lock(struct mtd_info *mtd, loff_t ofs, size_t len)
{
	return winbond_flash_do_unlock(mtd, ofs, len, 0);
}
...
static int probe_new_chip(struct mtd_info *mtd, __u32 base,
			  struct flchip *chips,
			  struct winbond_flash_private *private,
			  const struct winbond_flash_info *table, int table_size)
{
	__u32 mfr_id;
	__u32 dev_id;
	struct map_info *map = mtd->priv;
	struct winbond_flash_private temp;
	int i;
				...
				for (j = 0; j < private->numchips; j++) {
					...
					if ((mfr_id_other == mfr_id) &&
						...
						send_cmd(map, base,
							 CMD_RESET_DATA);
#else
						send_cmd(map, base,CMD_RESET);						
#endif
						return -1;
					}
				}

				if (private->numchips == MAX_WINBOND_CHIPS) {
					printk(KERN_WARNING
					       "%s: Too many flash chips "
					       "detected. Increase "
					       "MAX_WINBOND_CHIPS from %d.\n",
					       map->name, MAX_WINBOND_CHIPS);
...
static struct mtd_info *winbond_flash_probe(struct map_info *map)
{
	 ...
	 * probe is done.
	 */
	const struct winbond_flash_info table[] = {
	{
		mfr_id: MANUFACTURER_WINBOND,
		dev_id: W28J160BT,
		name: "WINBOND W28J160BT",
		size: 0x00200000,
		numeraseregions: 3,
		regions: {
			{ offset: 0x000000, erasesize: 0x02000, numblocks:  2 },
			{ offset: 0x004000, erasesize: 0x02000, numblocks:  6 },
			{ offset: 0x010000, erasesize: 0x10000, numblocks:  31 }
		}
	}, {
		mfr_id: MANUFACTURER_WINBOND,
		dev_id: W28J160TT,
		name: "WINBOND W28J160TT",
		size: 0x00200000,
		numeraseregions: 3,
		regions: {
			{ offset: 0x000000, erasesize: 0x10000, numblocks:  31 },
			{ offset: 0x1F0000, erasesize: 0x02000, numblocks:  6 },
			{ offset: 0x1FC000, erasesize: 0x02000, numblocks:  2 }
		}
	}, {
		mfr_id: MANUFACTURER_WINBOND,
		dev_id: W28J800BT,
		name: "WINBOND W28J800BT",
		size: 0x00100000,
		numeraseregions: 3,
		regions: {
			{ offset: 0x000000, erasesize: 0x02000, numblocks:  2 },
		...
		mfr_id: MANUFACTURER_WINBOND,
		dev_id: W28J800TT,
		name: "WINBOND W28J800TT",
		size: 0x00100000,
		numeraseregions: 3,
		regions: {
			{ offset: 0x000000, erasesize: 0x10000, numblocks:  15 },
			{ offset: 0x0F0000, erasesize: 0x02000, numblocks:  6 },
			{ offset: 0x0FC000, erasesize: 0x02000, numblocks:  2 }
		}
	}, 
#if 0	
	{//mcli 2004-11-29 15:23
		.mfr_id = MANUFACTURER_WINBOND_1,
		.dev_id = W19B320ABT,
		.name = "Winbond W19B320ABT",
		.size = 0x00400000,
		.numeraseregions = 2,
		.regions = {
...
#endif	
	{
		...
		mfr_id: MANUFACTURER_WINBOND,
		dev_id: W28J320TT,
		name: "WINBOND W28J320TT",
		size: 0x00400000,
		numeraseregions: 3,
		regions: {
			{ offset: 0x000000, erasesize: 0x10000, numblocks: 63 },
			{ offset: 0x3F0000, erasesize: 0x02000, numblocks:  6 },
			{ offset: 0x3FC000, erasesize: 0x02000, numblocks:  2 }
		}
	}
	};

	struct mtd_info *mtd;
	struct flchip chips[MAX_WINBOND_CHIPS];
	int table_pos[MAX_WINBOND_CHIPS];
	struct winbond_flash_private temp;
	struct winbond_flash_private *private;
	u_long size;
	unsigned long base;
	...
	printk("%s: Probing for WINBOND compatible flash...\n", map->name);

	if ((table_pos[0] = probe_new_chip(mtd, 0, NULL, &temp, table,
					   sizeof(table)/sizeof(table[0])))
	    == -1) {
		printk(KERN_WARNING
		       "%s: Found no WINBOND compatible device at location zero\n",
		       map->name);
		kfree(mtd);
	...
	mtd->name = map->name;
	mtd->erase = winbond_flash_erase;	
	mtd->read = winbond_flash_read;	
	mtd->write = winbond_flash_write;	
	mtd->sync = winbond_flash_sync;	
	mtd->suspend = winbond_flash_suspend;	
	mtd->resume = winbond_flash_resume;	
	mtd->lock = winbond_flash_lock;
	mtd->unlock = winbond_flash_unlock;
	...
	map->fldrv = &winbond_flash_chipdrv;
	MOD_INC_USE_COUNT;
...
static int winbond_flash_read(struct mtd_info *mtd, loff_t from, size_t len,
...
retry:
...
#else
	send_cmd(map, chip->start, CMD_WORD_WRITE);
#endif	
	...
	if (!times_left) {
		...
		__u32 verify;
#if 1 /*XFHU*/
		send_cmd(map, chip->start, CMD_RESET);
#endif		
...
static int winbond_flash_write(struct mtd_info *mtd, loff_t to , size_t len,
			   size_t *retlen, const u_char *buf)
{
	...
	*retlen = 0;
	...
	if (ofs & (map->bankwidth - 1)) {
		...
		if (map->bankwidth == 2) {
			//datum = *(__u16*)tmp_buf;
			datum = get_unaligned((__u16*)tmp_buf);//2004-11-2 11:14
		} else if (map->bankwidth == 4) {
			datum = *(__u32*)tmp_buf;
		} else {
	...
	while(len >= map->bankwidth) {
		...
		if (map->bankwidth == 1) {
			...
			//datum = *(__u16*)buf;
			datum = get_unaligned((__u16*)buf);//2004-11-2 11:14
		} else if (map->bankwidth == 4) {
			datum = *(__u32*)buf;
		} else {
			return -EINVAL;
...
#else	
	send_cmd(map, chip->start, CMD_BLOCK_ERASE1);
	send_cmd_to_addr(map, chip->start, CMD_BLOCK_ERASE2, adr);
#endif	
	timeo = jiffies + (HZ * 20);
...
static int winbond_flash_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	struct map_info *map = mtd->priv;
	struct winbond_flash_private *private = map->fldrv_priv;
...
static void winbond_flash_sync(struct mtd_info *mtd)
{
	struct map_info *map = mtd->priv;
	struct winbond_flash_private *private = map->fldrv_priv;
	int i;
...
static int winbond_flash_suspend(struct mtd_info *mtd)
{
printk("winbond_flash_suspend(): not implemented!\n");
	return -EINVAL;
}



static void winbond_flash_resume(struct mtd_info *mtd)
{
printk("winbond_flash_resume(): not implemented!\n");
}



static void winbond_flash_destroy(struct mtd_info *mtd)
{
	struct map_info *map = mtd->priv;
	struct winbond_flash_private *private = map->fldrv_priv;
	kfree(private);
...
int __init winbond_flash_init(void)
{
	
	DEBUGMTD("%s %s %d\n",__FILE__,__FUNCTION__,__LINE__);
	register_mtd_chip_driver(&winbond_flash_chipdrv);
	return 0;
}

void __exit winbond_flash_exit(void)
{
	unregister_mtd_chip_driver(&winbond_flash_chipdrv);
}

module_init(winbond_flash_init);
module_exit(winbond_flash_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Old MTD chip driver for WINBOND flash chips");
