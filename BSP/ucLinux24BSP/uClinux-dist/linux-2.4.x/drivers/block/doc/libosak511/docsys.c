...
void flWrite16bitUsing16bitsNoShift(volatile  byte FAR0 * win, word offset,Reg16bitType val)
{
  ...
  flwritew( val, (volatile void *)(win + offset) );
}
...
Reg16bitType flRead16bitUsing16bitsNoShift(volatile  byte FAR0 * win,word offset)
{
  ...
  return( flreadw( (volatile void *)(win + offset) ) );
}
...
{
  /* win[offset] = val; */
  flwriteb( val, (volatile void *)(win + offset) );
}
...
void flWrite8bitUsing16bitsNoShift(volatile  byte FAR0 * win, word offset,Reg8bitType val)
{
  ...
  flwritew( (word)val, (volatile void *)(win + offset) );  
#endif /* FL_BIG_ENDIAN */
}
...
{
  /* return win[offset]; */
  return( flreadb( (volatile void *)(win + offset) ) );
}
...
Reg8bitType flRead8bitUsing16bitsNoShift(volatile  byte FAR0 * win,word offset)
{
   ...
   return (( offset & 0x1      ) ?
    ...
    (Reg8bitType)(flreadw( (volatile void *)(win + (offset & 0xFFFE)) ) >> 8) :
    (Reg8bitType)flreadw( (volatile void *)(win + offset) ) );    
}
...
Reg8bitType flRead8bitUsing32bitsSingleShift(volatile  byte FAR0 * win,word offset)
{
   ...
   return (((offset & 0x1) == 0) ?
           ...
           (Reg8bitType)((((volatile dword FAR0 *)win)[offset>>1])>>16));
#else
   return (( offset & 0x1      ) ?
           (Reg8bitType)(((volatile dword FAR0 *)win)[offset>>1]>>8) :
...
void fl8bitDocReadNoShift(volatile  byte FAR0 * win,word offset,byte FAR1* dest,word count)
{
  ...
  *((byte FAR1*)addToFarPointer(dest, (count - 1)) ) = (byte)0;
  ...
  for(i=0;( i < count );i++)
    dest[i] = flreadb( (volatile void *)(win + offset + i) );
...
void fl8bitDocWriteNoShift(volatile  byte FAR0 * win,word offset,byte FAR1* src,word count)
{
  ...
  for(i=0;( i < count );i++)
    flwriteb( src[i], (volatile void *)(win + offset + i) );
}
...
void fl8bitDocSetNoShift(volatile  byte FAR0 * win,word offset,word count, byte val)
{
  ...
  register int i;
  for(i=0;( i < count );i++)
    flwriteb( val, (volatile void *)(win + offset + i) );
...
void fl16bitDocReadNoShift (volatile  byte FAR0 * win, word offset, byte FAR1 * dest, word count )
{
  ...
  *((byte FAR1*)addToFarPointer(dest, (count - 1)) ) = (byte)0;
   ...
   if( pointerToPhysical(dest) & 0x1 )
   {
      ...
      {
         /* tmp = *swin; */
         tmp = flreadw( (volatile void *)(swin) );
#ifdef FL_BIG_ENDIAN
         dest[i++] = (byte)(tmp>>8);
         dest[i++] = (byte)tmp;
#else
   ...
   {   /* mainstream case */
      ...
      if (flUse8Bit == 0)
      {
        for(i=0,count>>=1;( i < count );i++)
          ((word FAR1 *)dest)[i] = flreadw( (volatile void *)(swin) );
                             ...
                             byte FAR1 * src, word count )
{
...
#endif /* FL_BIG_ENDIAN */
         flwritew( tmp, (volatile void *)(swin) );
         /* *swin = tmp; */
       }
   }
   else /* mainstream case */
   {
      ...
      {
        for(i=0,count>>=1;( i < count );i++)
          flwritew( ((word FAR1 *)src)[i], (volatile void *)(swin) );
                                  ...
                                  word count , byte val)
{
   ...
   {
     for(i=0,count>>=1;( i < count );i++)
       flwritew( tmpVal, (volatile void *)(swin) );
...
#else
      dest[i] = (byte)flreadw( (volatile void *)(swin) );
      /* dest[i] = (byte)swin[i]; */
#endif /* FL_BIG_ENDIAN */
   }
}
...
#else
      /* *swin  = (word)src[i]; */
      flwritew( (word)src[i], (volatile void *)(swin) );
#endif /* FL_BIG_ENDIAN */
   }
}
