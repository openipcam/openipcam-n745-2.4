				   ...
				   byte if_cfg)
{
   switch(interleave)
   {
	 ...
	 DEBUG_PRINT(("Debug: 8-bit Flash Access (setDOCPlusBusType).\r\n"));
	 break;
      case 2: /* 2 flashes are interleaved */
	 busConfig |= FL_16BIT_FLASH_ACCESS;
	 DEBUG_PRINT(("Debug: 16-bit Flash Access (setDOCPlusBusType).\r\n"));
	 break;
      default:
   ...
   switch(if_cfg)
   {
	  ...
	  DEBUG_PRINT(("Debug: 8-bit DiskOnChip Access (setDOCPlusBusType).\r\n"));
	  break;
      case 16: /* 2 flashes are interleaved */
	  busConfig |= FL_16BIT_DOC_ACCESS;
	  DEBUG_PRINT(("Debug: 16-bit DiskOnChip Access (setDOCPlusBusType).\r\n"));
	  break;
      default:
...
static unsigned flDocWindowBaseAddress(byte socketNo, dword checkIde )
{
  dword                winSize;
  FLFlash              *flash;
  ...
  /* Init CompactMDOC+ IDE access data structures */
  ideInit( (word)checkIde );
  ...
  flash->win = (NDOC2window)physicalToPointer(checkIde,winSize,socketNo);
                        ...
                        FLBoolean reconstruct)
{
   ...
   if(blockMultiplier == 0) /* 2 blocks per byte */
   {
      ...
      for (floorInc = (start / bytesPerFloor) * NFDC21thisVars->floorSize +
           (BBT_MEDIA_OFFSET<<shift), last = (dword) length,
      ...
      while (length > flash->interleaving)
      {
        if(shift)
        {
           tmp = (*BYTE_ADD_FAR(buffer,length-1)) & /* lower lane  */
...
FLStatus    changeInterleave(FLFlash * flash, byte interNum)
{
    ...
    /* Call interleave changed event */
    if(CheckAccessMode != NULL)
       checkStatus(CheckAccessMode(interNum,(byte)flSocketNoOf(flash->socket),0));
...
FLStatus docPlusIdentify(FLFlash * flash)
{
  ...
  /* Changing to proper interleave */
  changeInterleave(flash, (byte)flash->interleaving);
...
FLStatus flRegisterIDEPLUSSOC( dword primaryIde, dword secondaryIde )
{
  FLSocket *socket;
  ...
  /* Try to register Ide DiskOnChip using */
  socket = flSocketOf(noOfSockets);
  ...
  socket->window.baseAddress = flDocWindowBaseAddress(
                                 (byte)(socket->volNo), primaryIde );
  if( (socket->window.baseAddress == 0) ) /* DiskOnChip not detected */
    socket->window.baseAddress = flDocWindowBaseAddress(
                                   (byte)(socket->volNo), secondaryIde );
 
  if( socket->window.baseAddress == 0 )
             ...
             dword highAddress)
{
    ...
    baseAddress = (dword) flDocWindowBaseAddress(0, lowAddress);
    if( baseAddress == 0L )
      baseAddress = (dword) flDocWindowBaseAddress(0, highAddress);
    socket->base = physicalToPointer(baseAddress, socket->size,0);
