...
{
#if !defined(FL_IDE_MDOCP) && !defined(FL_IDE_DOC)
  ((volatile word FAR0*)win)[offset>>1] = val;
#else
  ideWritew(offset,val);
#endif
}
...
#else
  ideWritew((offset & 0xFE),((word)val * 0x0101));
#endif /* FL_BIG_ENDIAN */
#endif
}
...
#else
	   (Reg8bitType)( ideReadw(offset & 0xFE) >> 8 ) :
	   (Reg8bitType)( ideReadw(offset) ));
#endif
}
...
#else
  if( offset == Nio ) {
    ideCpyReadb( offset, dest, count );
  }
  else {
    for( i = 0;( i < count ); i++ )
      *((unsigned char FAR1 *)dest + i) = ideReadb(offset + i);
  }
#endif
}
...
#else
        if( offset == Nio ) {
          ideCpyReadw( offset, dest, count );
        }
        else {
          for( i=0,count>>=1;( i < count ); i++ )
            *((unsigned short FAR1 *)dest + i) = ideReadw(offset + 2*i);
        }
#endif
      }
#endif /* USE_TFFS_COPY */
...
#else
      if( offset == Nio ) {
        ideCpyWritew( offset, src, count );
      }
	    else {
	      for( i=0,count>>=1;( i < count ); i++ )
	        ideWritew(offset + 2*i,((word FAR1 *)src)[i]);
      }
#endif
    }
#endif /* USE_TFFS_COPY */
...
void fl16bitDocReadNoShiftIgnoreHigher8bits(volatile byte FAR0 *win, word offset,
                                            byte FAR1 *dest, word count)
{
#if !defined(FL_IDE_MDOCP) && !defined(FL_IDE_DOC)
  volatile word FAR0 * swin = (volatile word FAR0 *)(win + offset);
#endif
...
#else
    if( offset == Nio )
	    dest[i] = (byte)ideReadw( offset );
    else
	    dest[i] = (byte)ideReadw( offset + 2*i );
#endif
#endif /* FL_BIG_ENDIAN */
   }
}
...
#else
    if( offset == Nio )
	    ideWritew( offset, ((word)src[i] * 0x0101) );
    else
	    ideWritew( (offset + 2*i),( (word)src[i] * 0x0101) );
#endif
#endif /* FL_BIG_ENDIAN */
  }
}
