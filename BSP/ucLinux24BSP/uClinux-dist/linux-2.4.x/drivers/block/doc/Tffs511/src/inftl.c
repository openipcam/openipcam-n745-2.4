...
static FLStatus verifySectors(Bnand vol, dword sectorCount)
{
   FLStatus    status;
   word        unitOffset;
  ...
  if ((vol.flags & QUICK_MOUNT) && (vol.firstMediaWrite == TRUE))
  {
     ...
     if (status==flOK)
     {
        ...
        if (status == flOK)
        {
           ...
           newVol->freeUnits        = vol.freeUnits         ;
           newVol->roverUnit        = vol.roverUnit         ;
           newVol->countsValid      = vol.countsValid       ;
           newVol->sectorsRead      = vol.sectorsRead       ;
           newVol->sectorsWritten   = vol.sectorsWritten    ;
           newVol->sectorsDeleted   = vol.sectorsDeleted    ;
           newVol->parasiteWrites   = vol.parasiteWrites    ;
           newVol->unitsFolded      = vol.unitsFolded       ;
           newVol->wearLevel_1      = vol.wearLevel.alarm   ;
           newVol->wearLevel_2      = vol.wearLevel.currUnit;
           newVol->eraseSum         = vol.eraseSum          ;
           newVol->validate         = QUICK_MOUNT_VALID_SIGN;
#if (defined(VERIFY_WRITE) || defined(VERIFY_ERASED_SECTOR))
           newVol->verifiedSectorNo = vol.verifiedSectorNo;
#else
           newVol->verifiedSectorNo = 0;
#endif /* VERIFY_WRITE || VERIFY_ERASED_SECTOR */
...
static FLBoolean checkQuickMountInfo(Bnand vol)
{
   ...
   if (status==flOK)
   {
      ...
      if (status == flOK)
      {
         ...
         if (newVol->validate == QUICK_MOUNT_VALID_SIGN)
         {
            vol.freeUnits          = (ANANDUnitNo)newVol->freeUnits;
            vol.roverUnit          = (ANANDUnitNo)newVol->roverUnit;
            vol.countsValid        = (ANANDUnitNo)newVol->countsValid;
            vol.sectorsRead        = newVol->sectorsRead;
            vol.sectorsWritten     = newVol->sectorsWritten;
            vol.sectorsDeleted     = newVol->sectorsDeleted;
            vol.parasiteWrites     = newVol->parasiteWrites;
            vol.unitsFolded        = newVol->unitsFolded;
            vol.wearLevel.alarm    = (word)newVol->wearLevel_1;
            vol.wearLevel.currUnit = (ANANDUnitNo)newVol->wearLevel_2;
            vol.eraseSum           = newVol->eraseSum;
#if (defined(VERIFY_WRITE) || defined(VERIFY_ERASED_SECTOR))
            vol.verifiedSectorNo   = newVol->verifiedSectorNo;
#endif /* VERIFY_WRITE || VERIFY_ERASED_SECTOR */
...
static FLStatus formatINFTL(unsigned volNo, TLFormatParams *fp, FLFlash *flash)
{
  ...
  dword       startUnit,endUnit; /* Default protection start and end units */
...
/*  fp->flags |= TL_QUICK_MOUNT_FORMAT; */
  ...
  *-----------------------------------------------------*/
  ...
  if(fp->flags & FL_LEAVE_BINARY_AREA) /* Previous Boot area is kept */
  {
     ...
     fp->noOfBinaryPartitions = (volumeHeader - (BNANDVolumeHeaderRecord *) firstVolumePtr);
  ...
  for (floorNo=skipedUnits/floorGarantiedUnits[0];floorNo<noOfFloors;floorNo++)
  {
     for (index=0;index<flash->totalProtectedAreas;index++)
     {
        if(binaryFloorOfDPS[index] == floorNo)
           continue; // break; /* Belongs to a binary partition that must be left */

        /* Set start and end units for default protection */
        ...
        /* Call MTD set protction routine */
        status = flash->protectionSet(flash,(byte)index,
                 WRITE_PROTECTED,                           /* Write protect */
                 (startUnit) << flash->erasableBlockSizeBits, /* Low address   */
                 (endUnit) << flash->erasableBlockSizeBits, /* High address  */
      ...
      /* Copy protection key and type */
        ...
        if (LE4(volumeHeader->flags) & PROTECTABLE)
        {
           status = flash->protectionSet(flash,(byte)LE4(volumeHeader->protectionArea),
              ...
              (byte FAR1 *)protectionKey,DO_NOT_COMMIT_PROTECTION,floorNo);
           if (status != flOK)
           {
