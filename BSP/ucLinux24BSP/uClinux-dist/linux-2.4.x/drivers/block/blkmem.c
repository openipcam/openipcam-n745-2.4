...
#endif
#if defined(CONFIG_ARCH_WINBOND)
#ifndef ROMFS_BASE
...
int __init blkmem_init( void )
{
  ...
  for(i=0;i<arenas;i++) {
    ...
    //if(i==0)
    //	arena[i].address=GetImgAddr(0xD00000);//GetImgAddr(0x700000);
    //#else
	if(i==0)
#ifndef ROMFS_BASE
  ...
  for(i=0;i<arenas;i++) {
	...
	for(i=0;i<arenas;i++) {
		...
		sprintf(name,"rom%d",i);
		devfs_register (NULL, name, DEVFS_FL_DEFAULT,
					MAJOR_NR, i,
					S_IFBLK | S_IRUSR | S_IWUSR,
					&blkmem_fops,
					NULL);
		if (devfs_register_blkdev(MAJOR_NR, name, &blkmem_fops )) {
