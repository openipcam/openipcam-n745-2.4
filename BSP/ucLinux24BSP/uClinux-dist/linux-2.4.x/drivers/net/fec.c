...
/* ------------------------------------------------------------------------- */
/* The Level one LXT972 is used on the Arcturus Networks uC5272 dimm        */
...
*  UC5272 - MII interrupt not currently connected to any interrupt ...MaTed
*/
#if !defined(CONFIG_BOARD_UC5272)
static void
...
mii_discover_phy3(uint mii_reg, struct net_device *dev)
{
	...
	{
		printk(" -- %s\n", phy_info[i]->name);
#if defined(CONFIG_M5272) && !defined(CONFIG_BOARD_UC5272)
  ...
  case SIOCDEVPRIVATE+1:          /* for binary compat, remove in 2.5 */
	...
	if (data->value_in == SIOC_MIIREG_QUERY)
	{
	  ...
	  if (fep->fec_ioctl_reg == FEC_IOCTL_LXT972_STAT2)
	  {
		...
		if (link_stat != fep->link)
		{
		  if (FEC_DEBUG > 4)
		  {
			printk("   *****: STAT2 RELINK: %s link_stat=%d fep->link=%d\n ",
				   dev->name, link_stat, fep->link);
		  }
		  mii_relink( dev);
		  if (FEC_DEBUG > 4)
		  {
			printk("   *****: STAT2 RELINKED: %s link_stat=%d fep->link=%d\n ",
				   dev->name, link_stat, fep->link);
		  }
		}
		if (link_stat)
