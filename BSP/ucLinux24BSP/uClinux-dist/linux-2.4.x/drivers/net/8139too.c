...
#if defined(CONFIG_MTD_NETtel) // || defined(CONFIG_SH_SECUREEDGE5410)
/*
 *	On boards missing the external PROM we need to work around an
 *	intermitant interrupt latch up problem. To do this we use a
 *	fast poll clock to simulate the ethernet interrupts.
 *	A poll of 500Hz gives very good performamce results.
 */
#define	POLLINTR	500
...
/*
 *	We keep a linked list of ethernet devices to be poll interrupted.
 */
void *rtl8139_mmcrp;
struct net_device *rtl8139_head;
#endif /* CONFIG_MTD_NETtel || CONFIG_SH_SECUREEDGE5410 */
...
#if defined(CONFIG_MTD_NETtel) // || defined(CONFIG_SH_SECUREEDGE5410)
static void rtl8139_poll(int irq, void *dev, struct pt_regs *regs);
#else
...
#endif
#if POLLINTR
	if (rtl8139_head == NULL) {
		rtl8139_mmcrp = (void *) ioremap_nocache(0xfffef000, 4096);
		if (rtl8139_mmcrp != NULL) {
			volatile unsigned char *pit1map;

			printk("rtl8139: %dHz clock timer on IRQ 15\n", POLLINTR);

			if (request_irq(15, rtl8139_poll, 0, "rtl8139 poll clock", dev) != 0)
				printk("8139TOO: request_irq() failed?\n");
			...
			*pit1map = 0x0a;
			...
			outb(0x74, 0x43);
			outb((SC520_CLOCK / POLLINTR) & 0xff, 0x41);
			outb(((SC520_CLOCK / POLLINTR) >> 8) & 0xff, 0x41);
		...
		if (tp->next_timer == 0) {
			struct net_device *nxtdev;
			struct rtl8139_private *nxtp;

			printk("rtl8139: inserting into poll chain\n");
			nxtdev = rtl8139_head;
			do {
				nxtp = (struct rtl8139_private *) nxtdev->priv;
				nxtdev = nxtp->next_dev;
			} while (nxtdev != NULL);
			nxtp->next_dev = dev;
			nxtp->next_timer = 1;
			nxtp = dev->priv;
 ...
 */
static void rtl8139_poll(int irq, void *dev_instance, struct pt_regs *regs)
{
	...
	while (dev != NULL) {
		...
		tp = (struct rtl8139_private *) dev->priv;
		dev = tp->next_dev;
	}
}
#endif /* POLLINTR */
