...
static void *usb_audio_parsecontrol(struct usb_device *dev, unsigned char *buffer, unsigned int buflen, unsigned int ctrlif)
{
	...
	for (i = 0; i < p1[7]; i++) {
		...
		j = p1[8+i];
		printk("p1[7]:%d,j %d,config->bNumInterfaces:%d\n",p1[7],j,config->bNumInterfaces);
		...
		int alt=1;
		if (iface->altsetting[alt].bNumEndpoints < 1) {
			printk(KERN_ERR "usbaudio: device %d audiocontrol interface %u interface %u altsetting %d has no endpoint\n",
			       dev->devnum, ctrlif, j, alt);
			alt++;
			if (iface->altsetting[alt].bNumEndpoints < 1) {
				printk(KERN_ERR "usbaudio: device %d audiocontrol interface %u interface %u altsetting %d has no endpoint\n",
			       dev->devnum, ctrlif, j,alt);
		...
		if (iface->altsetting[alt].endpoint[0].bEndpointAddress & USB_DIR_IN) {
...
ret:
	...
	printk("usb_audio_parsecontrol: usb_audio_state at %p\n", s);
	return s;
}
...
static void usb_audio_disconnect(struct usb_device *dev, void *ptr)
{
	...
	for(list = s->audiolist.next; list != &s->audiolist; list = list->next) {
		...
		if (as->dev_audio >= 0) {
			...
			printk("usbaudio: unregister dsp 14,%d\n", as->dev_audio);
	...
	for(list = s->mixerlist.next; list != &s->mixerlist; list = list->next) {
		...
		if (ms->dev_mixer >= 0) {
			...
			printk("usbaudio: unregister mixer 14,%d\n", ms->dev_mixer);
