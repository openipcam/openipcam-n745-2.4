...
#else
	#define NTFS_GETU16(p)     ((*(char *)(p))|(*(char *)(((char *)(p))+1))<<8)
	#define NTFS_GETU24(p)     (NTFS_GETU16(p) | \
				   (NTFS_GETU8(((char*)(p)) + 2) << 16))
	#define NTFS_GETU32(p)     ((*(char *)(p))|(*(char *)(((char *)(p))+1))<<8| \
					(*(char *)(((char *)(p))+2))<<16 | (*(char *)(((char *)(p))+3))<<24)
	#define NTFS_GETU40(p)     (NTFS_GETU32(p) | \
				   (NTFS_GETU8(((char*)(p)) + 4)) << 32)
	#define NTFS_GETU48(p)     (NTFS_GETU32(p) | \
				   (NTFS_GETU16(((char*)(p)) + 4)) << 32)
	#define NTFS_GETU56(p)     (NTFS_GETU32(p) | \
				   (NTFS_GETU24(((char*)(p)) + 4)) << 32)
	#define NTFS_GETU64(p)     (NTFS_GETU32(p) |(NTFS_GETU32(((char*)(p)) + 4)) << 32)
#endif
...
#else
	#define NTFS_PUTU16(p,v)     do {				\
					NTFS_GETU8(p) = NTFS_GETU8(v);	\
					NTFS_GETU8((char *)(p)+1) = NTFS_GETU8((char *)(v)+1);\
					}while(0)
	#define NTFS_PUTU24(p,v)     NTFS_PUTU16(p, (v) & 0xFFFF);\
	                             NTFS_PUTU8(((char*)(p)) + 2, (v) >> 16)
	#define NTFS_PUTU32(p,v)     do {				\
					NTFS_GETU8(p) = NTFS_GETU8(v);	\
					NTFS_GETU8((char *)(p)+1) = NTFS_GETU8((char *)(v)+1);\
					NTFS_GETU8((char *)(p)+2) = NTFS_GETU8((char *)(v)+2);\
					NTFS_GETU8((char *)(p)+3) = NTFS_GETU8((char *)(v)+3);\
					}while(0)
	#define NTFS_PUTU64(p,v)     NTFS_PUTU32(p, v);\
				     NTFS_PUTU32(((char *)(p)+4), (v) >> 32)	
#endif
...
#else
	#define NTFS_GETS16(p)       ((*(char *)(p))|(*(char *)(((char*)(p))+1))<<8)
	#define NTFS_GETS24(p)       (NTFS_GETU24(p) < 0x800000 ? \
						(int)NTFS_GETU24(p) : \
						(int)(NTFS_GETU24(p) - 0x1000000))
	#define NTFS_GETS32(p)       ((*(char *)(p))|(*(char *)(((char*)(p))+1))<<8 | \
					(*(char *)(((char *)p)+2))<<16 | (*(char *)(((char*)(p))+3))<<24)
	#define NTFS_GETS40(p)       (((ntfs_s64)NTFS_GETU32(p)) | \
				     (((ntfs_s64)NTFS_GETS8(((char*)(p)) + 4)) << 32))
	#define NTFS_GETS48(p)       (((ntfs_s64)NTFS_GETU32(p)) | \
