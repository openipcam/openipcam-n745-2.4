...
const char *yaffs_guts_c_version =
    "$Id: yaffs_guts.c,v 1.1.1.1 2007-07-31 05:18:10 andy Exp $";
		...
		for (chunkInBlock = 0, oldChunk = block * dev->nChunksPerBlock;
			...
			if (yaffs_CheckChunkBit(dev, block, chunkInBlock)) {
				...
				if (!object) {
					T(YAFFS_TRACE_ERROR,
					  (TSTR
					   ("page %d in gc no object,but it is good! "//zswan add it
					    TENDSTR), oldChunk));
				}
...
int yaffs_GutsInitialise(yaffs_Device * dev)
{
	...
	if ( dev->isYaffs2) {
		...
		yaffs_Scan(dev);//zswan change it
		}
