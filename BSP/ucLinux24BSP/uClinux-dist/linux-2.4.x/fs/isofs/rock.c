 ...
 repeat:
  {
	...
	if(rr->u.TF.flags & TF_CREATE) 
	  inode->i_ctime = iso_date(rr->u.TF.times[cnt++].time, 0);//iso_date(times[cnt++].time, 0);
	if(rr->u.TF.flags & TF_MODIFY) 
	  inode->i_mtime = iso_date(rr->u.TF.times[cnt++].time, 0);//iso_date(times[cnt++].time, 0);
	if(rr->u.TF.flags & TF_ACCESS) 
	  inode->i_atime = iso_date(rr->u.TF.times[cnt++].time, 0);//iso_date(times[cnt++].time, 0);
	...
	{int slen;
	 ...
	 slen = rr->len - 5;
	 slp = (struct SL_component *)&rr->u.SL.link;//__link;
