 ...
 *
 * $Id: pptp_ctrl.c,v 1.1.1.1 2006-08-14 02:32:37 andy Exp $
 */
...
void Send_UDP_MSG(char* buf,int len)
{
   int sockfd;
   ...
   servaddr.sin_addr.s_addr=inet_addr("192.168.1.252");
...
//char debug_buf[50];
  ...
  //assert(conn && conn->call);
	//write(2,"debug----- 4\n",20);
//	Send_UDP_MSG(conn->write_buffer,conn->write_size);
	
	
	retval=send(conn->inet_sock, conn->write_buffer, conn->write_size,0);


	
  //retval = write(conn->inet_sock, conn->write_buffer, conn->write_size);
	//sprintf(debug_buf,"write retval=%d\n",retval);
	//write(2,debug_buf,20);
  if (retval < 0) { /* error. */
...
//	write(2,"In pptp_read_some\n",20);
  ...
  retval = read(conn->inet_sock, 
	...
	//sprintf(debug_buf,"read retval=%d\n",retval);
//	write(2,debug_buf,20);
  if (retval < 0) {
...
//	dump_packet(conn->read_buffer,20);
...
void pptp_dispatch_ctrl_packet(PPTP_CONN * conn, void * buffer, size_t size) {
  ...
  case PPTP_START_CTRL_CONN_RPLY:
	...
	DEBUG_MSG("Client connection established.\n");
...
//	dump_packet(buffer,sizeof(struct pptp_out_call_rply));
	callid= ntoh16(packet->call_id_peer);
      if (!vector_search(conn->call, (int) callid, &call)) {
	DEBUG_MSG("PPTP_OUT_CALL_RPLY received for non-existant call.");
	break;
      }
      if (call->call_type!=PPTP_CALL_PNS) {
	DEBUG_MSG("Ack!  How did this call_type get here?"); /* XXX? */
	break; 
      }
	...
	if (packet->result_code!=1) {
	  ...
	  int err = packet->error_code;
          DEBUG_MSG("Our outgoing call request [callid %d] has not been accepted.", (int) callid);
          DEBUG_MSG("Reply result code is %d '%s'. Error code is %d, Cause code is %d",
	      packet->result_code,
	    ...
	    DEBUG_MSG("Error is '%s', Error message: '%s'",
	  ...
	  DEBUG_MSG("Outgoing call established (call ID %u, peer's call ID %u).\n",
