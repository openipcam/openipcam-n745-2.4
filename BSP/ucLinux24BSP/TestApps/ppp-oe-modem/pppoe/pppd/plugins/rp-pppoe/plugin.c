...
"$Id: plugin.c,v 1.1.1.1 2006-08-14 02:32:37 andy Exp $";
...
int m_pppoe_id=0;
int old_pppoe_id;
#define _PATH_ETHOPT         _ROOT_PATH "/etc/ppp/pppoe-options." 
//end
...
PPPOEConnectDevice(void)
{
	...
	pppd_info.session_num=ppp_session_number;//mcli
    m_pppoe_id=pppd_info.session_num;//mcli

    /* Make the session socket */
    conn->sessionSocket = socket(AF_PPPOX, SOCK_STREAM, PX_PROTO_OE);
    if (conn->sessionSocket < 0) {
	set_pppd_status(CONNECTION_OFF); //mcli
	...
	memcpy(pppd_info.ser_mac, conn->peerEth, ETH_ALEN);//mcli
	memcpy(ser_mac, conn->peerEth, ETH_ALEN);//mcli
	...
	PPPD_DEBUG(printf("remote_number=%s\n",remote_number));
...
//mcli    info("RP-PPPoE plugin version %s compiled against pppd %s",
//mcli 		RP_VERSION, VERSION);
}
...
rp_fatal(char const *str)
{
    ...
    sendPADT(conn, buf);
	set_pppd_status(CONNECTION_OFF);//mcli
	pppd_set_run(PPPD_STOP_RUN);//mcli
