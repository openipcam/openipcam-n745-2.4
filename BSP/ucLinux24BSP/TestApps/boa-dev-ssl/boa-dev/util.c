...
char *FixupDocumentRoot(char *pcDocumentRoot)
{
	...
	char *pcDir_Back;
	int iDirBufSize;
	iDirBufSize = MAX_PATH_LENGTH * 2 + 1;

	if (pcDocumentRoot == NULL) pcDocumentRoot = ".";
	if ((pcDir_Back = (char *)malloc(iDirBufSize)) == NULL)
	{
		PRINT_MEM_OUT;
		return NULL;
	}
	printf("Start http server at: [%s]\n", pcDocumentRoot);
	if (getcwd(pcDir_Back, iDirBufSize) == NULL || chdir(pcDocumentRoot) == -1)
	{
		free(pcDir_Back);
		return NULL;
	}

	if ((pcDir = (char *)malloc(iDirBufSize)) == NULL)
	{
		PRINT_MEM_OUT;
		chdir(pcDir_Back);
		free(pcDir_Back);
	...
	if (getcwd(pcDir, iDirBufSize) == NULL)
	{
		chdir(pcDir_Back);
		free(pcDir_Back);
		free(pcDir);
...
#endif
		my_syslog(LOG_ERR, "Out of buffer space");
		return 0;
	}
...
//		write(1, req->buffer + req->buffer_start,
//				bytes_to_write);
		bytes_written = (*req->write)(req,
...
int get_mac_from_IP(request *req){
	FILE *fd;
	char acRemoteIP[16];
	char tmp[81];
	int len = 81;
	...
	IP2String(req->remote_ip_addr.s_addr, acRemoteIP);
	...
	while(fgets(tmp, len, fd)){
		...
		str = strtok(tmp, " \t");
		if(strcmp(str, acRemoteIP) == 0){ /*found IP address*/
			for(i=0;i<3;i++){  /*Move along until the HW address*/
				str = strtok(NULL, " \t");
			}
			//strcpy(MAC, str);
			j = sscanf(str, "%x:%x:%x:%x:%x:%x", iMac, iMac+1, iMac+2, iMac+3, iMac+4, iMac+5);
			for (i=0; i<j; i++)
				req->acRemoteMac[i] = (char)iMac[i];
...
int do_mac_crap(request *req)
{
	...
	{
		if (i > 0) acMac[i*3-1] = ':';
		Char2Hex(req->acRemoteMac[i], &acMac[i*3]);
	}
	printf("%s - %s\n", acRemoteIP, acMac);
#endif
}
